...
export function MeetInvite() {
  ...
  if (!m) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
      ...
      <p className="text-sm text-jax-gray-4 dark:text-jax-gray-2 mb-4">This meeting link is invalid or has been deleted.</p>
      <Link to="/" className="text-jax-blue hover:underline text-sm">Back to City Hall</Link>
    </div>
  )
    ...
    <div className="min-h-screen bg-jax-light dark:bg-jax-ink">
      <header className="bg-jax-navy text-jax-light py-4 px-6">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <div>
            <div className="text-xs uppercase tracking-widest text-jax-sky">City of Jacksonville</div>
            <div className="text-sm font-semibold">Video meeting invite</div>
          </div>
        </div>
      </header>
        ...
        <div className="bg-white dark:bg-jax-navy-deep/40 border border-jax-gray-1 dark:border-jax-blue/20 rounded-lg p-8 shadow-sm">
          ...
          {when && (
            <div className="flex items-center gap-1.5 text-sm text-jax-gray-4 dark:text-jax-gray-2 mb-1">
              ...
              {when.toLocaleString([], { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' })}
              {m.duration_minutes && ` · ${m.duration_minutes} minutes`}
              {dial && <span className="ml-2 text-jax-success font-semibold uppercase tracking-wider text-[10px]">starts soon</span>}
            </div>
          )}
          ...
          <div className="mt-6 space-y-3">
            <label className="block">
              <div className="text-xs font-semibold uppercase tracking-wider text-jax-gray-4 dark:text-jax-gray-2 flex items-center gap-1 mb-1">
                <Users className="h-3 w-3" /> Your name (so attendees can identify you)
              </div>
              <input
                value={name}
            ...
            >
              <Video className="h-4 w-4" /> Join meeting <ArrowRight className="h-4 w-4" />
            </button>
          </div>
        </div>

        <div className="mt-4 text-center text-xs text-jax-gray-3">
          <MapPin className="inline h-3 w-3 mr-1" /> Hosted by City of Jacksonville Citizen Services · 630-CITY · Powered by Liftori
        </div>
      </main>
    </div>
  )
